...
export default function CreateCoursePage() {
  ...
  const [sections, setSections] = useState<Section[]>([
    { id: "sec-1", title: "섹션 1: 소개", lessons: [] }
  ])
  ...
  const addSection = () => {
    const newSection: Section = {
      id: `sec-${Date.now()}`,
      title: `섹션 ${sections.length + 1}: 새로운 섹션`,
      lessons: []
    }
    ...
    <div className="container px-4 md:px-8 py-12 max-w-5xl">
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex gap-2">
          ...
          {step === 2 ? (
            <Button className="bg-white text-zinc-950 hover:bg-zinc-200">강의 게시하기</Button>
          ) : (
      ...
      <div className="mb-8 flex items-center gap-4">
        <div className={`flex items-center gap-2 ${step >= 1 ? 'text-indigo-500' : 'text-zinc-500'}`}>
          <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 ${step >= 1 ? 'border-indigo-500 bg-indigo-500/10' : 'border-zinc-700'}`}>1</div>
          <span className="font-medium">기본 정보</span>
        </div>
        <div className="w-12 h-px bg-zinc-800" />
        <div className={`flex items-center gap-2 ${step >= 2 ? 'text-indigo-500' : 'text-zinc-500'}`}>
          <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 ${step >= 2 ? 'border-indigo-500 bg-indigo-500/10' : 'border-zinc-700'}`}>2</div>
        ...
        <div className="grid gap-8 lg:grid-cols-[1fr_300px]">
            ...
            <Card className="bg-zinc-900/50 border-zinc-800">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input placeholder="강의의 특징을 한 문장으로 설명해주세요" className="bg-zinc-950/50 border-zinc-800" />
                </div>
                <div className="space-y-2">
          ...
          <div className="space-y-6">
            <Card className="bg-zinc-900/50 border-zinc-800">
              ...
              <CardContent>
                <div className="aspect-video bg-zinc-950 border-2 border-dashed border-zinc-800 rounded-lg flex flex-col items-center justify-center text-zinc-500 hover:border-zinc-600 hover:text-zinc-400 transition-colors cursor-pointer">
            ...
            <Card className="bg-zinc-900/50 border-zinc-800">
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input type="number" placeholder="0" className="bg-zinc-950/50 border-zinc-800" />
                </div>
                <div className="flex items-center gap-2">
                  <input type="checkbox" id="free" className="rounded border-zinc-800 bg-zinc-950" />
                  <label htmlFor="free" className="text-sm">무료 강의</label>
                </div>
              </CardContent>
            </Card>
          </div>
        ...
        <div className="max-w-3xl mx-auto space-y-8">
          <Card className="bg-zinc-900/50 border-zinc-800">
            <CardHeader>
              ...
              <CardDescription>섹션과 강의를 추가하여 커리큘럼을 완성하세요.</CardDescription>
            </CardHeader>
                ...
                <div key={section.id} className="border border-zinc-800 rounded-lg p-4 bg-zinc-950/30">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button variant="ghost" size="sm" className="h-8 text-red-400">삭제</Button>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3 pl-4 md:pl-8">
                      ...
                      <div key={lesson.id} className="bg-zinc-900 rounded border border-zinc-800 overflow-hidden">
                        <div className="flex items-center justify-between p-3 bg-zinc-900/50">
                          <div className="flex items-center gap-3">
                            ...
                            {lesson.type === 'video' ? <Video className="w-4 h-4 text-blue-400" /> : <FileQuestion className="w-4 h-4 text-green-400" />}
                            <span className="text-sm font-medium">{lesson.title}</span>
                          </div>
                          <div className="flex gap-1">
                            <Button variant="ghost" size="sm" className="h-8 w-8 p-0" onClick={() => toggleEditLesson(section.id, lesson.id)}>
                        ...
                        {lesson.isEditing && (
                          <div className="p-4 border-t border-zinc-800 bg-zinc-950/30 space-y-4">
                            <div className="space-y-2">
                              <label className="text-xs font-medium text-muted-foreground">강의 제목</label>
                              <Input 
                                value={lesson.title} 
                            ...
                            {lesson.type === 'video' && (
                              <div className="space-y-2">
                                ...
                                <div className="flex gap-2">
                                  <Input type="file" className="bg-zinc-950 border-zinc-800 text-sm file:bg-zinc-800 file:text-zinc-200 file:border-0 file:mr-4 file:py-1 file:px-2 file:rounded-md" />
                                  <Button size="sm" variant="secondary">업로드</Button>
                                </div>
                              </div>
                            )}
                                  ...
                                  {lesson.questions && lesson.questions.length > 0 ? (
                                    <div className="space-y-2">
                                      {lesson.questions.map((q, qIndex) => (
                                        <div key={q.id} className="flex items-center justify-between p-2 rounded bg-zinc-900 border border-zinc-800 text-sm">
                                          <div className="flex items-center gap-2">
                                            <span className="text-muted-foreground">{qIndex + 1}.</span>
                                            <span>{q.question}</span>
                                            <Badge variant={q.answer === 'O' ? 'default' : 'destructive'} className="ml-2">
                                              {q.answer}
                                            </Badge>
                                          </div>
                                          <Button variant="ghost" size="sm" className="h-6 w-6 p-0 text-red-400" onClick={() => removeQuestion(section.id, lesson.id, q.id)}>
                                    ...
                                    <p className="text-xs text-muted-foreground italic">등록된 문제가 없습니다.</p>
                                  )}
                                </div>
                                  ...
                                  <div className="flex items-center gap-4">
                                    ...
                                    >
                                      <div className="flex items-center space-x-2">
                                        ...
                                        <Label htmlFor="r-o">O (정답)</Label>
                                      </div>
                                      <div className="flex items-center space-x-2">
                                        <RadioGroupItem value="X" id="r-x" />
                                        <Label htmlFor="r-x">X (오답)</Label>
                                      </div>
                                    </RadioGroup>
                                    <Button size="sm" onClick={() => addQuestionToQuiz(section.id, lesson.id)} className="ml-auto h-8">
                                      문제 추가
                                    </Button>
                                  </div>
                                </div>
                            ...
                            <div className="flex justify-end pt-2">
                              <Button size="sm" onClick={() => toggleEditLesson(section.id, lesson.id)} className="bg-indigo-600 hover:bg-indigo-700 text-white">
                    ...
                    <div className="flex gap-2 pt-2">
                      <Button variant="outline" size="sm" onClick={() => addLesson(section.id, 'video')} className="flex-1 border-dashed border-zinc-700 hover:bg-zinc-800 text-zinc-400">
                        <Video className="w-4 h-4 mr-2" />
                        영상 강의 추가
                      </Button>
                      <Button variant="outline" size="sm" onClick={() => addLesson(section.id, 'quiz')} className="flex-1 border-dashed border-zinc-700 hover:bg-zinc-800 text-zinc-400">
